...
Bsweep = c/(2*radarRangeResolution);
...
for i=1:length(t)         
    ...
    %Receiver Signal
    Mix(i) = times(Tx(i), Rx(i)); %Element-by-element multiplication of fi objects
end
...
single_sided_sig_fft1 = sig_fft1(1:Nr/2);
...
convRDM_ker = conv2(RDMpow, kernel, 'valid') ./ num_cells;
threshold = pow2db(convRDM_ker)+offset;
[row,col] = size(threshold);

RDM = RDM(Td+Gd+1:Td+Gd+row, Tr+Gr+1:Tr+Gr+col);
RDM(RDM < threshold) = 0;
RDM(RDM >= threshold) = 1;
